classdef UI < handle
    ...
    % functions for opening and reading various files:
        ...
        function openHDF5(this)
            ...
            % Daten einlesen (abh�ngig von Einstellungen)
                        ...
                        switch FileType
                            ...
                            case 'in-vivo-dual'
                                ...
                                this.modes{i} = InvivoMode(this, double(reader.data.sisa_1270.data),...
                                                         ...
                                                         reader.meta.sisa.int_time, reader, i);
                            otherwise
                                warndlg(['Kann das Dateiformat ' FileType ' nicht �ffnen!']);
                        end
                        i = i + 1;
                    case {'fluo', 'spec'}
...
%                 [name, filepath] = uigetfile({[this.openpath filenames ';*.diff;*.asc;*.state'];[this.openpath filenames '*.h5;*.diff;*.asc;*.state']}, 'Dateien ausw�hlen', 'MultiSelect', 'on');
                [name, filepath] = uigetfile({[filenames '*.diff;*.asc;*.state'], 'DB-Beschr�nkt'; '*.h5;*.diff;*.asc;*.state', 'alle'}, 'Dateien ausw�hlen', this.openpath, 'MultiSelect', 'on');
           
            else
                [name, filepath] = uigetfile({[this.openpath '*.h5;*.diff;*.asc;*.state']}, 'Dateien ausw�hlen', 'MultiSelect', 'on');
            end
...
%             addpath([get_executable_dir '\..\src']);
        ...
        function open_db_cb(this, varargin)
            this.loadini();
            % get path of file from user
            [name, filepath] = uigetfile({[this.dbpath '*.db']}, 'Dateien ausw�hlen', 'MultiSelect', 'on');
            if (~ischar(name) && ~iscell(name)) || ~ischar(filepath) % no file selected
                return
            end
                ...
                for i = 1:length(name)
                    ...
                    if i > 1
                        if length(d) > size(data, 5)
                            d = d(1:size(data, 5));
                            this.update_infos(['    |    L�nge der Daten ungleich in ' name{i}]);
        ...
        function open_nsTAS_cb(this,varargin)
            this.loadini();
            [name, filepath] = uigetfile({[this.open_nsTAS_path '*.txt;*.diff']}, 'Dateien ausw�hlen', 'MultiSelect', 'on');
            if (~ischar(name) && ~iscell(name)) || ~ischar(filepath) % no file selected
                return
            end
